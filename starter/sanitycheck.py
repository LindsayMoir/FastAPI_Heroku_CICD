...
def run_sanity_check(test_dir):
    ...
    if not test_functions_for_post:
        ...
        if len(test_functions_for_post) == 1:
            ...
            print(
                FAIL_COLOR +
                "Make sure you have two test cases for the POST() method."
                + "\nOne test case for EACH of the possible inferences \
                (results/outputs) of the ML model.\n")
            SANITY_TEST_PASSING = False
        ...
        if len(
            ...
            print(
                FAIL_COLOR +
                "You do not have TWO separate test cases, one for each \
    ...
    print(
        WARN_COLOR +
        "You should check your work against the rubric to ensure you meet \
        the criteria.")
