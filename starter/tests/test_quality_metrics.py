...
def test_quality_metrics(y_test=None, predictions=None):
    """Check if the quality_metrics function generates the expected files."""
    # Get config
    ...
    assert os.path.exists(conf_matrix_path), f"File {conf_matrix_path} \
        was not created."
    assert os.path.exists(roc_curve_path), f"File {roc_curve_path} \
        was not created."
    ...
    conf_matrix_mtime = pd.to_datetime(os.path.getmtime(conf_matrix_path),
                                       unit='s')
    roc_curve_mtime = pd.to_datetime(os.path.getmtime(roc_curve_path),
                                     unit='s')

    # This is unix time which is utc. Just subtract 7 hours (pst) to get
    # the correct time.
    ...
    assert start_time <= conf_matrix_mtime <= end_time, f"\
        File {conf_matrix_path} not created within the appropriate time frame."
    assert start_time <= roc_curve_mtime <= end_time, f"\
        File {roc_curve_path} not created within the appropriate time frame."
